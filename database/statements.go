...
// Sets
const GET_ALL_SETS = "select sets.id, user_id, artists.id, artists.name, locations.id, locations.name, sets.date, sets.rating, sets.genre " +
	"FROM sets INNER JOIN artists ON artists.id = sets.artist_id " +
	"INNER JOIN locations ON locations.id = sets.location_id;"
const GET_ALL_SETS_FOR_USER_FORMAT = "select sets.id, user_id, artists.id, artists.name, locations.id, locations.name, sets.date, sets.rating, sets.genre " +
	"FROM sets INNER JOIN artists ON artists.id = sets.artist_id " +
	"INNER JOIN locations ON locations.id = sets.location_id " +
	"WHERE user_id=%d;"
const IS_SET_UNIQUE_QUERY = "select COUNT(*) FROM sets where user_id = ? and artist_id = ? and location_id = ? and date = ?"
const INSERT_NEW_SET = `insert into sets (user_id, artist_id, location_id, date, rating, genre) values(?,?,?,?,?,?);`
